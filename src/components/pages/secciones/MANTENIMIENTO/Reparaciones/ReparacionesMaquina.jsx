...
// Datos de ejemplo (front-only, sin backend todavía)
const SEED = [
  { _id: "1", fecha: "2026-06-10", descripcion: "Cambio de aceite y filtros", urgencia: "media", estado: "pendiente", detalle: "", repuestos: [{ nombre: "Filtro de aceite", costo: 18000, observaciones: "Original" }] },
  { _id: "2", fecha: "2026-06-05", descripcion: "Reparación de sistema hidráulico", urgencia: "alta", estado: "pendiente", detalle: "Pierde aceite por la manguera del cilindro principal.", repuestos: [] },
  { _id: "3", fecha: "2026-05-28", descripcion: "Engrase general", urgencia: "baja", estado: "terminada", detalle: "", repuestos: [] },
];

function ReparacionesMaquina({ maquina, onVolver }) {
  const nombre = maquina?.maquina ?? "—";

  const [trabajos, setTrabajos] = useState(SEED);
  ...
  const onSubmit = (data) => {
    if (editando) {
      setTrabajos((prev) => prev.map((t) => (t._id === editando ? { ...t, ...data } : t)));
    } else {
      setTrabajos((prev) => [
        { _id: crypto.randomUUID(), estado: "pendiente", detalle: "", repuestos: [], ...data },
        ...prev,
      ]);
    }
    cerrarForm();
    Swal.fire({ icon: "success", title: editando ? "Trabajo actualizado" : "Trabajo guardado", timer: 1200, showConfirmButton: false });
  };
  ...
  const toggleEstado = (t) =>
    setTrabajos((prev) => prev.map((x) => (x._id === t._id ? { ...x, estado: x.estado === "terminada" ? "pendiente" : "terminada" } : x)));

  /* ── Urgencia ── */
  const toggleUrgencia = (t) =>
    setTrabajos((prev) => prev.map((x) => {
      if (x._id !== t._id) return x;
      const idx = URGENCIAS.indexOf(x.urgencia ?? "baja");
      return { ...x, urgencia: URGENCIAS[(idx + 1) % URGENCIAS.length] };
    }));

  /* ── Detalle ── */
  const abrirDetalle = (t) => { setDetalleId(t._id); setDetalleTexto(t.detalle ?? ""); setShowDetalle(true); };
  const guardarDetalle = () => {
    setTrabajos((prev) => prev.map((t) => (t._id === detalleId ? { ...t, detalle: detalleTexto } : t)));
  ...
  const guardarRepuestos = () => {
    const lista = repuestosLista.map(({ nombre, costo, observaciones }) => ({ nombre, costo: parsearPeso(costo), observaciones }));
    setTrabajos((prev) => prev.map((t) => (t._id === repuestosId ? { ...t, repuestos: lista } : t)));
    setShowRepuestos(false);
    Swal.fire({ icon: "success", title: "Repuestos guardados", timer: 1000, showConfirmButton: false });
  };
  ...
  return (
    <Container className="py-4">
      ...
      <div className="text-center w-75 mx-auto" style={{ marginTop: "2rem", marginBottom: "1.5rem" }}>
        <h3 className="fw-bold mb-0">Reparaciones: {nombre}</h3>
      </div>
...
export default ReparacionesMaquina;
